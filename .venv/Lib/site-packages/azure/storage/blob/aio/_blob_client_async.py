...
class BlobClient(AsyncStorageAccountHostsMixin, BlobClientBase):  # pylint: disable=too-many-public-methods
    ...
    async def upload_blob(
        ...
        :paramtype: ~azure.storage.blob.blob.aio.BlobLeaseClient
        :keyword ~datetime.datetime if_modified_since:
            A DateTime value. Azure expects the date value passed in to be UTC.
            If timezone is included, any non-UTC datetimes will be converted to UTC.
            If a date is passed in without timezone info, it is assumed to be UTC.
    ...
    async def download_blob(self, offset=None, length=None, **kwargs):
        ...
        :paramtype lease: ~azure.storage.blob.blob.aio.BlobLeaseClient or str
        :keyword ~datetime.datetime if_modified_since:
            A DateTime value. Azure expects the date value passed in to be UTC.
            If timezone is included, any non-UTC datetimes will be converted to UTC.
            If a date is passed in without timezone info, it is assumed to be UTC.
        ...
        :rtype: ~azure.storage.blob.blob.aio.StorageStreamDownloader
    ...
    async def start_copy_from_url(self, source_url, metadata=None, incremental_copy=False, **kwargs):
        ...
        :keyword destination_lease:
            ...
            destination blob. If the request does not include the lease ID or it is not
            valid, the operation fails with status code 412 (Precondition Failed).
        :paramtype destination_lease: ~azure.storage.blob.blob.aio.BlobLeaseClient or str
        :keyword source_lease:
            Specify this to perform the Copy Blob operation only if
            the lease ID given matches the active lease ID of the source blob.
        :paramtype source_lease: ~azure.storage.blob.blob.aio.BlobLeaseClient or str
        :keyword int timeout:
            The timeout parameter is expressed in seconds.
        :keyword ~azure.storage.blob.blob.PremiumPageBlobTier premium_page_blob_tier:
            A page blob tier value to set the blob to. The tier correlates to the size of the
    ...
    async def acquire_lease(self, lease_duration=-1, lease_id=None, **kwargs):
        ...
        :rtype: ~azure.storage.blob.blob.aio.BlobLeaseClient
